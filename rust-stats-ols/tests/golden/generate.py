"""Generate golden reference values for rust-stats-ols parity tests.

Run manually:
    python3 rust-stats-ols/tests/golden/generate.py
...
def rank_deficient_input():
    """Only saves the input — statsmodels silently uses a pseudoinverse
    and we want the rust side to error instead."""
    rng = np.random.default_rng(7)
    n = 25
    x_base = rng.standard_normal((n, 2))
    x = np.column_stack([x_base[:, 0], x_base[:, 1], x_base[:, 0]])
